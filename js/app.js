...
function initMap() {
    ...
    setTimeout(() => {
        ...
        events.forEach(event => {
            ...
            const marker = L.marker([event.location.lat, event.location.lng], { icon }).addTo(mapInstance);
...
function createMapPopup(event) {
    ...
    let imageHtml = event.media ? `<img src="${event.media}" alt="${event.title}" class="map-popup-image">` : '';
