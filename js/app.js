...
async function checkGithubAuth() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('GitHub auth success:', githubUser.login)
      return githubUser
    } else {
...
document.addEventListener('DOMContentLoaded', async () => {
  const user = await checkGithubAuth()
  if (user) {
    showGithubStatus(user)
  }
  initNavigation()
  initTheme()
  renderGroupsList()
...
function showGithubStatus(user) {
  ...
  statusDiv.style.cssText = `
    ...
    `
  statusDiv.innerHTML = `
        <img src="${user.avatar_url}" style="width:20px;height:20px;border-radius:50%;">
